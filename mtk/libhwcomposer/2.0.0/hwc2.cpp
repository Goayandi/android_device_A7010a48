...
int32_t checkMirrorPath(const std::vector<sp<HWCDisplay> >& displays, bool *ultra_scenario);
...
void HWCBuffer::afterPresent(const bool& is_disp_connected, const bool& is_ct)
{
...
String8 HWCLayer::toString8()
{
    ...
    String8 ret;
    ret.appendFormat("id:%" PRIu64 " v:%d acq:%d hnd:%p,%d,%" PRIu64 " w:%d,%d h:%d,%d f:%u sz:%d z:%u c:%s(%s,%s%d,%s,%d) s[%.1f,%.1f,%.1f,%.1f]->d[%d,%d,%d,%d] t:%d d(s%d,b%d)",
        getId(),
        isVisible(),
        getAcquireFenceFd(),
        getHandle(),
...
void HWCLayer::validate()
{
    ...
    if (getBlend() == HWC2_BLEND_MODE_NONE &&
        getPrivateHandle().format == HAL_PIXEL_FORMAT_RGBA_8888)
    {
...
void HWCLayer::afterPresent(const bool& is_disp_connected)
{
    ...
    if (getHwcBuffer() != nullptr)
        getHwcBuffer()->afterPresent(is_disp_connected, isClientTarget());
...
void HWCDisplay::validate()
{
    ...
    m_vali_present_state = HWC_VALI_PRESENT_STATE_VALIDATE;
    m_is_validated = true;
    ...
    const bool& isDisabled, const int32_t& disp_width, const int32_t& disp_height)
{
        ...
        isDisabled)
    {
        ...
        src_crop.right = disp_width;
        src_crop.bottom = disp_height;
...
void HWCDisplay::beforePresent(const int32_t num_validate_display)
{
    ...
    if (NULL != job)
    {
        job->color_transform = m_color_transform;
    ...
    {
        clearAllFences();
        HWC_LOGE("job is null");
    }
}

void HWCDisplay::present()
{
    m_vali_present_state = HWC_VALI_PRESENT_STATE_PRESENT;
...
void HWCDisplay::afterPresent()
{
    ...
    {
        auto& layer = kv.second;
        layer->afterPresent(isConnected());
    }
...
void HWCDisplay::removePendingRemovedLayers()
{
    ...
    auto iter = std::begin(m_layers);
    while (iter != std::end(m_layers))
    {
        auto& layer = iter->second;
        bool is_removed = layer->isPendingRemoved();

        if (!layer->isClientTarget() && is_removed && layer->isVisible())
        {
        ...
        if (is_removed)
        {
            ...
            HWC_LOGV("(%" PRIu64 ") %s: destroy layer id(%" PRIu64 ")", getId(), __func__, layer->getId());
#else
            HWC_LOGD("(%" PRIu64 ") %s: destroy layer id(%" PRIu64 ")", getId(), __func__, layer->getId());
#endif
...
void HWCDisplay::buildVisibleLayersSortedByZ()
{
    ...
    {
        auto& layer = kv.second;
        if (!layer->isPendingRemoved() && !layer->isClientTarget())
        {
    ...
    if (HWCMediator::getInstance().getOvlDevice(HWC_DISPLAY_PRIMARY)->isDispRpoSupported() &&
        m_visible_layers.size() > 1 &&
        m_visible_layers[0]->getSFCompositionType() == HWC2_COMPOSITION_SOLID_COLOR)
    {
        HWC_LOGI("to skip the most bottom dim layer");
        m_visible_layers.erase(m_visible_layers.begin());
    }
...
int32_t HWCDisplay::destroyLayer(const hwc2_layer_t& layer)
{
    HWC_LOGD("(%" PRIu64 ") %s layer:% " PRIu64, getId(), __func__, layer);

    AutoMutex l(m_pending_removed_layers_mutex);
    m_layers[layer]->remove();
    return HWC2_ERROR_NONE;
}
...
void HWCDisplay::dump(String8* dump_str)
{
    ...
    dump_str->appendFormat(" visible_layer:%zu commit_layer:%zu\n",
    ...
    , m_callback_refresh_data(nullptr)
{
    ...
    */
    ...
    property_get("debug.hwc.is_skip_validate", value, "-1");
    if (-1 != atoi(value))
    {
    ...
    int err = property_set("debug.sf.hwc_pid", buf);
    if (err < 0) {
...
void HWCMediator::deviceDump(hwc2_device_t* /*device*/, uint32_t* outSize, char* outBuffer)
{
    ...
    else
    {
        ...
        property_get("debug.hwc.av_grouping", value, "-1");
        if (-1 != atoi(value))
        {
        ...
        property_get("debug.hwc.dump_buf_cont", value, "-1");
        if ('-' != value[0])
        {
            if (value[0] == 'M' || value[0] == 'U' || value[0] == 'C')
            {
        ...
        property_get("debug.hwc.dump_buf_log_enable", value, "-1");
        if (-1 != atoi(value))
        {
            Platform::getInstance().m_config.dump_buf_log_enable = atoi(value);
        }

        property_get("debug.hwc.fill_black_debug", value, "-1");
        if (-1 != atoi(value))
        {
        ...
        property_get("debug.hwc.color_transform", value, "-1");
        if (-1 != atoi(value))
        {
            Platform::getInstance().m_config.support_color_transform = atoi(value);
        }

        property_get("debug.hwc.enable_rpo", value, "-1");
        if (1 == atoi(value))
        {
        ...
        property_get("debug.hwc.rpo_ui_max_src_width", value, "-1");
        if (-1 != atoi(value))
        {
            Platform::getInstance().m_config.rpo_ui_max_src_width = atoi(value);
        }

        property_get("debug.hwc.mdp_scale_percentage", value, "-1");
        ...
        property_get("debug.hwc.extend_mdp_cap", value, "-1");
        if (-1 != atoi(value))
        {
        ...
        dump_str.appendFormat("\n[HWC Property]\n");
#ifndef MTK_USER_BUILD
        dump_str.appendFormat("  force_full_invalidate(debug.hwc.forceFullInvalidate):%d\n", Platform::getInstance().m_config.force_full_invalidate);
        dump_str.appendFormat("  wait_fence_for_display(debug.hwc.waitFenceForDisplay):%d\n", Platform::getInstance().m_config.wait_fence_for_display);
        dump_str.appendFormat("  rgba_rotate(debug.hwc.rgba_rotate):%d\n", Platform::getInstance().m_config.enable_rgba_rotate);
        dump_str.appendFormat("  rgba_rotate(debug.hwc.rgbx_scaling):%d\n", Platform::getInstance().m_config.enable_rgbx_scaling);
        dump_str.appendFormat("  compose_level(debug.hwc.compose_level):%d, ", Platform::getInstance().m_config.compose_level);
        dump_str.appendFormat("  mirror_state(debug.hwc.mirror_state):%d\n", Platform::getInstance().m_config.mirror_state);

        dump_str.appendFormat("  prexformUI(debug.hwc.prexformUI):%d\n", Platform::getInstance().m_config.prexformUI);
        dump_str.appendFormat("  log_level(persist.debug.hwc.log):%c, ", Debugger::getInstance().getLogThreshold());
        dump_str.appendFormat("  skip_period_log(debug.hwc.skip_log):%d\n", Debugger::m_skip_log);
        dump_str.appendFormat("  mhl_output(debug.hwc.mhl_output):%d\n", Platform::getInstance().m_config.format_mir_mhl);

        dump_str.appendFormat("  profile_level(debug.hwc.profile_level):%d\n", DisplayManager::m_profile_level);
        dump_str.appendFormat("  mir_scale_ratio(debug.hwc.mir_scale_ratio):%f\n", Platform::getInstance().m_config.mir_scale_ratio);
        dump_str.appendFormat("  force_pri_secure(debug.hwc.force_pri_secure):%d\n", DisplayManager::getInstance().m_data[HWC_DISPLAY_PRIMARY].secure);
        dump_str.appendFormat("  force_vir_secure(debug.hwc.force_vir_secure):%d\n", DisplayManager::getInstance().m_data[HWC_DISPLAY_VIRTUAL].secure);
        dump_str.appendFormat("  ext layer:%d(debug.hwc.ext_layer)\n", Platform::getInstance().m_config.enable_smart_layer);

        dump_str.appendFormat("  trigger_by_vsync(debug.hwc.trigger_by_vsync):%d\n", m_features.trigger_by_vsync);
        dump_str.appendFormat("  AV_grouping(debug.hwc.av_grouping):%d\n", Platform::getInstance().m_config.av_grouping);
        dump_str.appendFormat("  DumpBuf(debug.hwc.dump_buf):%c-%d, DumpBufCont(debug.hwc.dump_buf_cont):%c-%d log:%d\n",
        ...
        dump_str.appendFormat("  fill_black_debug(debug.hwc.fill_black_debug):%d\n", Platform::getInstance().m_config.fill_black_debug);
        dump_str.appendFormat("  Always_Setup_Private_Handle(debug.hwc.always_setup_priv_hnd):%d\n", Platform::getInstance().m_config.always_setup_priv_hnd);
        dump_str.appendFormat("  wdt_ioctl(debug.hwc.wdt_ioctl):%d\n", Platform::getInstance().m_config.wdt_ioctl);
        dump_str.appendFormat("  only_wfd_by_hwc(debug.hwc.only_wfd_by_hwc):%d\n", Platform::getInstance().m_config.only_wfd_by_hwc);
        dump_str.appendFormat("  blitdev_for_virtual(debug.hwc.blitdev_for_virtual):%d\n", Platform::getInstance().m_config.blitdev_for_virtual);

        dump_str.appendFormat("  is_skip_validate(debug.hwc.is_skip_validate):%d\n", Platform::getInstance().m_config.is_skip_validate);
        dump_str.appendFormat("  support_color_transform(debug.hwc.color_transform):%d\n", Platform::getInstance().m_config.support_color_transform);
        dump_str.appendFormat("  mdp_scaling_percentage(debug.hwc.mdp_scale_percentage):%.2f\n", Platform::getInstance().m_config.mdp_scale_percentage);
        dump_str.appendFormat("  ExtendMDP(debug.hwc.extend_mdp_cap):%d\n", Platform::getInstance().m_config.extend_mdp_capacity);
        dump_str.appendFormat("\n");
#else // MTK_USER_BUILD
...
int32_t  HWCMediator::displayGetAttribute(
    hwc2_device_t* device,
    hwc2_display_t display,
    ...
    int32_t* out_value)
{
	...
	switch (attribute)
        ...
        case HWC2_ATTRIBUTE_VSYNC_PERIOD:
            *out_value = 16666666.6667;
            break;

        case HWC2_ATTRIBUTE_DPI_X:
            *out_value = 403411.764706;
            break;

        case HWC2_ATTRIBUTE_DPI_Y:
            *out_value = 403041.322314;
            break;
...
int32_t  HWCMediator::displayGetName(
    hwc2_device_t* device,
    hwc2_display_t display,
    uint32_t* out_lens,
    char* out_name)
...
{
    *out_num_types = 0; //HDR not supported
    return HWC2_ERROR_NONE;
}
    ...
    int32_t* out_retire_fence)
{
         ...
         m_displays[display]->getValiPresentState() == HWC_VALI_PRESENT_STATE_VALIDATE))
    {
        ...
        {
            buildVisibleLayerForAllDisplay();
            ...
            setValiPresentStateOfAllDisplay(HWC_VALI_PRESENT_STATE_CHECK_SKIP_VALI);
            if (checkSkipValidate() == true)
                m_need_validate = HWC_SKIP_VALIDATE_SKIP;
            else
                m_need_validate = HWC_SKIP_VALIDATE_NOT_SKIP;
        }

        if (m_need_validate == HWC_SKIP_VALIDATE_NOT_SKIP)
        {
            return HWC2_ERROR_NOT_VALIDATED;
        }
        else
        {
            ...
            setValiPresentStateOfAllDisplay(HWC_VALI_PRESENT_STATE_VALIDATE_DONE);
        }
    }
    ...
    if (m_displays[display]->getValiPresentState() == HWC_VALI_PRESENT_STATE_VALIDATE_DONE)
    {
        ...
        for (auto& hwc_display : m_displays)
        {
            ...
            hwc_display->beforePresent(m_validate_seq);
        }
    ...
    m_displays[display]->setValiPresentState(HWC_VALI_PRESENT_STATE_PRESENT_DONE);
    return HWC2_ERROR_NONE;
}
    ...
    int32_t mode)
{
    ...
    if (mode < 0 || mode > 9) {
    	HWC_LOGE("Could not find mode: %d", mode);
    	return HWC2_ERROR_BAD_PARAMETER;
    }
...
{
    HWC_LOGI("(%" PRIu64 ") %s hint:%d", display, __func__, hint);
    if (!m_displays[display]->isConnected())
    {
        HWC_LOGE("%s: the display(%" PRIu64 ") is not connected", __func__, display);
        return HWC2_ERROR_BAD_DISPLAY;
...
void Hrt::fillLayerConfigList(const std::vector<sp<HWCDisplay> >& displays)
{
    for (auto& display : displays)
    {
        ...
        for (auto& layer : layers)
        {
            ...
            layer_config->src_fmt       = mapDispInFormat(layer->getPrivateHandle().format);
...
void Hrt::run(std::vector<sp<HWCDisplay> >& displays, const bool& is_skip_validate)
{
    if (0 == isEnabled())
    {
...
static int32_t findLimitedVideo(const std::vector<sp<HWCLayer> >& layers)
{
    for (auto& layer : layers)
    {
...
static bool isMirrorList(const std::vector<sp<HWCLayer> >& src_layers,
                         const std::vector<sp<HWCLayer> >& sink_layers,
                         ...
                         const int32_t& sink_disp)
{
    ...
    std::set<uint64_t> src_layers_alloc_id;
    std::set<uint64_t> src_layers_sideband;
    ...
    for (auto& layer : src_layers)
    {
        ...
        if (layer->getCompositionType() != HWC2_COMPOSITION_SIDEBAND)
        {
            ...
            src_layers_alloc_id.insert(hnd.alloc_id);
        }
        // todo: check sidebandStream
    }

    HWC_LOGV("src_layers_hnds size:%d", src_layers_alloc_id.size());
    ...
    for (auto& layer : sink_layers)
    {
        if (layer->getCompositionType() != HWC2_COMPOSITION_SIDEBAND)
        {
            ...
            if (src_layers_alloc_id.find(hnd.alloc_id) != src_layers_alloc_id.end())
            {
                logger.printf("T2 ");
                ret = true;
                break;
    ...
    uint32_t* out_num_requests)
{
    ...
    std::vector<sp<HWCLayer> > changed_comp_types;
    const std::vector<int32_t>& prev_comp_types = m_displays[display]->getPrevCompTypes();
    {
        ...
        for (size_t i = 0; i < layers.size(); ++i)
        {
            ...
            auto& display_frame = layers[i]->getDisplayFrame();
            auto& src_crop = layers[i]->getSourceCrop();
#ifndef MTK_USER_BUILD
    ...
    *out_num_requests = 0;
    ...
    ++m_validate_seq;
    m_present_seq = 0;
    m_displays[display]->setValiPresentState(HWC_VALI_PRESENT_STATE_VALIDATE_DONE);
...
void HWCMediator::buildVisibleLayerForAllDisplay()
{
    ...
    for (auto& hwc_display : m_displays)
    {
        ...
        hwc_display->buildVisibleLayersSortedByZ();
...
void HWCMediator::setValiPresentStateOfAllDisplay(const HWC_VALI_PRESENT_STATE& val)
{
    for (size_t i = 0; i < m_displays.size(); ++i)
    {
        if (!m_displays[i]->isValid())
        ...
        m_displays[i]->setValiPresentState(val);
    }
}
...
void calculateMdpDstRoi(sp<HWCLayer> layer, const double& mdp_scale_percentage, const int32_t& z_seq)
{
    if (!HWCMediator::getInstance().getOvlDevice(HWC_DISPLAY_PRIMARY)->isDispRpoSupported())
    {
        ...
        layer->getHwlayerType() == HWC_LAYER_TYPE_MM_HIGH))
    {
        if (layer->getLayerCaps() & MDP_ROT_LAYER)
        {
        ...
        if (layer->getLayerCaps() & MDP_RSZ_LAYER)
        {
            const int32_t src_crop_width = WIDTH(layer->editMdpDstRoi());
            const int32_t src_crop_height = HEIGHT(layer->editMdpDstRoi());
            const int32_t disp_frame_width = WIDTH(layer->getDisplayFrame());
            const int32_t disp_frame_height = HEIGHT(layer->getDisplayFrame());

            const int32_t max_src_width_of_disp_rsz = HWCMediator::getInstance().getOvlDevice(HWC_DISPLAY_PRIMARY)->getCapsInfo()->rsz_in_max[0];
            const PrivateHandle& priv_hnd = layer->getPrivateHandle();

            if (fabs(mdp_scale_percentage - 1.0f) < 0.05f ||
                src_crop_width >= disp_frame_width ||
                src_crop_height >= disp_frame_height)
            {
                // downscale or disabling disp rsz
                layer->editMdpDstRoi() = layer->getDisplayFrame();
            }
            else if (src_crop_width > max_src_width_of_disp_rsz)
            {
                ...
                {
                    // for ui
                    if (src_crop_width <= Platform::getInstance().m_config.rpo_ui_max_src_width &&
                        src_crop_height > 20)
                    {
                        // downscale first by MDP and then upscale by Disp RSZ
                        const int& disp_frame_left    = layer->getDisplayFrame().left;
                        const int& disp_frame_top     = layer->getDisplayFrame().top;
                        const int& disp_frame_right   = layer->getDisplayFrame().right;
                        const int& disp_frame_bottom  = layer->getDisplayFrame().bottom;
                        const double ui_scale_down_percentage = static_cast<double>(max_src_width_of_disp_rsz) / src_crop_width;

                        layer->editMdpDstRoi().left   = disp_frame_left;
                        layer->editMdpDstRoi().top    = disp_frame_top;
                        layer->editMdpDstRoi().right  = disp_frame_left + max_src_width_of_disp_rsz;
                        layer->editMdpDstRoi().bottom = disp_frame_top + src_crop_height * ui_scale_down_percentage;
                    }
                    else
                    {
                        layer->editMdpDstRoi() = layer->getDisplayFrame();
            ...
            else
            {
                const double max_width_scale_percentage_of_mdp =
                    static_cast<double>(max_src_width_of_disp_rsz - src_crop_width) / (disp_frame_width - src_crop_width);

                const double final_mdp_scale_percentage = std::min(max_width_scale_percentage_of_mdp, mdp_scale_percentage);
                ...
                layer->editMdpDstRoi().left   = disp_frame_left;
                layer->editMdpDstRoi().top    = disp_frame_top;
                layer->editMdpDstRoi().right = disp_frame_left +
                    src_crop_width * (1 - final_mdp_scale_percentage) + disp_frame_width * final_mdp_scale_percentage;
                layer->editMdpDstRoi().bottom = disp_frame_top +
                    src_crop_height * (1 - final_mdp_scale_percentage) + disp_frame_height * final_mdp_scale_percentage;
            }
        }
        else
        {
    ...
    else
    {
        const PrivateHandle& priv_hnd = layer->getPrivateHandle();
        if ((priv_hnd.format == HAL_PIXEL_FORMAT_RGBA_8888 ||
            priv_hnd.format == HAL_PIXEL_FORMAT_BGRA_8888) && (layer->getLayerCaps() & MDP_ROT_LAYER))
        {
            ...
            switch (layer->getTransform())
            {
                case HAL_TRANSFORM_ROT_90:
                case HAL_TRANSFORM_ROT_270:
                    layer->editMdpDstRoi().right  = layer->editMdpDstRoi().left + HEIGHT(layer->getSourceCrop());
                    layer->editMdpDstRoi().bottom = layer->editMdpDstRoi().top  + WIDTH(layer->getSourceCrop());
                    break;

                default:
                    layer->editMdpDstRoi().right  = layer->editMdpDstRoi().left + WIDTH(layer->getSourceCrop());
                    layer->editMdpDstRoi().bottom = layer->editMdpDstRoi().top + HEIGHT(layer->getSourceCrop());
                    break;
            }
        }
        else
