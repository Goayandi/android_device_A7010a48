...
struct LayerListInfo
{
    ...
    // size of layer_config* is equal to size of layer list
    layer_config* layer_config_list;
};
...
public:
    ...
    bool decideDirtyAndFlush(int dpy, const PrivateHandle* priv_handle,
        int idx, int dirty, int type, const int64_t& hwc2_layer_id, const int32_t& layer_caps);

    // setSessionMode() is used to set display session mode
    void setSessionMode(int dpy, bool mirrored);
    DISP_MODE getSessionMode(const int& dpy);
