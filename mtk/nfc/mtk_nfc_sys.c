...
typedef enum
{
    nfc_mtk_Msgringbuf_main,
    nfc_mtk_Msgringbuf_service,
    nfc_mtk_Msgringbuf_halaosp,
    nfc_mtk_Msgringbuf_maxsize,
}nfc_mtk_Msgringbuf_id_et;
...
typedef struct
{
    MTK_NFC_MSG_RING_BUF RingBuf;
    MTK_NFC_MSG_RING_BUF * pRingBuf;
    MTK_NFC_MSG_T * ring_buffer[MTK_NFC_MSG_RING_SIZE]; //pointer array
    INT32 cnt;
    MTK_NFC_MUTEX_E MsgQ_MutexId;
    MTK_NFC_TASKID_E TaskId;
    MTK_NFC_EVENT_E EventId;
}nfc_msg_ringBuf_st;
/***************************************************************************** 
 * Extern Area
 *****************************************************************************/ 
...
nfc_msg_ringBuf_st nfc_MsgRing[nfc_mtk_Msgringbuf_maxsize];
...
static INT32 mtk_nfc_sys_socket_read(INT32 sockfd, UINT8 *pRecvBuff, INT32 i4RecvLen)
{
   ...
   NFCD("mtk_nfc_sys_socket_read...(fd:%d, pRecvBuff: %p, i4RecvLen:%d)\n", sockfd, pRecvBuff, i4RecvLen);
...
{
    nfc_mtk_Msgringbuf_id_et cnt1;
    nfc_msg_ringBuf_st* pMsgRing;

    NFCD("mtk_nfc_sys_msg_initialize msgbuf num:%d , Mutexnum %d , event num %d\n",nfc_mtk_Msgringbuf_maxsize,MTK_NFC_MUTEX_MAX_NUM,MTK_NFC_EVENT_END);
    
    for(cnt1 = nfc_mtk_Msgringbuf_main; cnt1 < nfc_mtk_Msgringbuf_maxsize;++cnt1)
    {
        pMsgRing  = &nfc_MsgRing[cnt1];
        pMsgRing->pRingBuf = &pMsgRing->RingBuf;
        pMsgRing->pRingBuf->start_buffer = &pMsgRing->ring_buffer[0];
        pMsgRing->pRingBuf->end_buffer = &pMsgRing->ring_buffer[MTK_NFC_MSG_RING_SIZE-1];
        pMsgRing->pRingBuf->next_write = pMsgRing->pRingBuf->start_buffer;
        pMsgRing->pRingBuf->next_read = pMsgRing->pRingBuf->start_buffer;
        pMsgRing->cnt = 0;
        switch(cnt1)
        {
        case nfc_mtk_Msgringbuf_main:
            pMsgRing->TaskId = MTK_NFC_TASKID_MAIN;
            pMsgRing->MsgQ_MutexId = MTK_MUTEX_MSG_Q;
            pMsgRing->EventId = MTK_NFC_EVENT_2MAIN;
            
            break;
        case nfc_mtk_Msgringbuf_service:
            pMsgRing->TaskId = MTK_NFC_TASKID_SERVICE;
            pMsgRing->MsgQ_MutexId = MTK_MUTEX_SERVICE_MSG_Q;
            pMsgRing->EventId = MTK_NFC_EVENT_2SERV;
            break;
        case nfc_mtk_Msgringbuf_halaosp:
            pMsgRing->TaskId = MTK_NFC_TASKID_AOSP_HAL;
            pMsgRing->MsgQ_MutexId = MTK_MUTEX_AOSPHAL_MSG_Q;
            pMsgRing->EventId = MTK_NFC_EVENT_2AOSPHAL;
            break;
        }
    }
#if 0    
...
#if 1
    if ((MTK_NFC_TASKID_MAIN == task_id)||(MTK_NFC_TASKID_SERVICE == task_id)||(MTK_NFC_TASKID_AOSP_HAL == task_id))
    {
        nfc_msg_ringBuf_st* pMsgRing;

        switch(task_id)
        {
        case MTK_NFC_TASKID_MAIN:
            pMsgRing = &nfc_MsgRing[nfc_mtk_Msgringbuf_main];
            break;
        case MTK_NFC_TASKID_SERVICE:
            pMsgRing = &nfc_MsgRing[nfc_mtk_Msgringbuf_service];
            break;
        case MTK_NFC_TASKID_AOSP_HAL:
            pMsgRing = &nfc_MsgRing[nfc_mtk_Msgringbuf_halaosp];
            break;
        }
        mtk_nfc_sys_mutex_take(pMsgRing->MsgQ_MutexId);
        
        /* msg queue full check */
        if (pMsgRing->cnt== MTK_NFC_MSG_RING_SIZE)
        {
            mtk_nfc_sys_mutex_give(pMsgRing->MsgQ_MutexId);
            NFCD("Send message to main, full\n");
            return MTK_NFC_ERROR;
        }
        
        if ( pMsgRing->pRingBuf != NULL)
        {
            *(pMsgRing->pRingBuf->next_write) = (MTK_NFC_MSG_T*)msg;
        
            pMsgRing->pRingBuf->next_write++;

            // Wrap check the input circular buffer
            if ( pMsgRing->pRingBuf->next_write > pMsgRing->pRingBuf->end_buffer )
            {
                pMsgRing->pRingBuf->next_write = pMsgRing->pRingBuf->start_buffer;
            }
            ...
            #ifdef SUPPORT_BLOCKING_READ_MECHANISM
            if (MTK_NFC_SUCCESS != mtk_nfc_sys_event_set(pMsgRing->EventId))
            {
...
{
    nfc_mtk_Msgringbuf_id_et MsgQId;
    if (msg != NULL)
    {
#if 1
        if ((MTK_NFC_TASKID_MAIN == task_id)||(MTK_NFC_TASKID_SERVICE == task_id)||(MTK_NFC_TASKID_AOSP_HAL == task_id))
        {
            ...
            {
            case MTK_NFC_TASKID_MAIN:
                MsgQId = nfc_mtk_Msgringbuf_main;
                break;
            case MTK_NFC_TASKID_SERVICE:
                MsgQId = nfc_mtk_Msgringbuf_service;
                break;
            case MTK_NFC_TASKID_AOSP_HAL:
                MsgQId = nfc_mtk_Msgringbuf_halaosp;
                
                break;
            }
            pMsgRing = &nfc_MsgRing[MsgQId];
            ...
            mtk_nfc_sys_mutex_take(pMsgRing->MsgQ_MutexId);
             ...
             {   
                 #ifdef SUPPORT_BLOCKING_READ_MECHANISM
                 if (MTK_NFC_SUCCESS != mtk_nfc_sys_event_wait(pMsgRing->EventId, pMsgRing->MsgQ_MutexId))
                 {
                     NFCD("mtk_nfc_sys_event_wait,fail , MsgQId %d\n",MsgQId);
                     mtk_nfc_sys_mutex_give(pMsgRing->MsgQ_MutexId);
                     return MTK_NFC_ERROR;
                 }
                 else
                 {
                     NFCD("mtk_nfc_sys_event_wait,ok,MsgQid %d,cnt,%d\n",MsgQId ,pMsgRing->cnt);                 
                 }
                 #else
                 mtk_nfc_sys_mutex_give(pMsgRing->MsgQ_MutexId);                 
                 return MTK_NFC_ERROR;
             ...
             if ((*(pMsgRing->pRingBuf->next_read) == NULL)||(pMsgRing->cnt <= 0))
             {
                 mtk_nfc_sys_mutex_give(pMsgRing->MsgQ_MutexId);
                 return MTK_NFC_ERROR;
             }    
            (*msg) = *(pMsgRing->pRingBuf->next_read);
            
            
            pMsgRing->pRingBuf->next_read++;
            
            // Wrap check output circular buffer
            if ( pMsgRing->pRingBuf->next_read > pMsgRing->pRingBuf->end_buffer )
            {
                pMsgRing->pRingBuf->next_read = pMsgRing->pRingBuf->start_buffer;
            }
...
)
{
   ...
   {
        NFCD("mtk_nfc_sys_gpio_write : node,%d , ioctl fail: %d, %s\n", gInterfaceHandle, errno, strerror(errno));
   }
...
VOID 
mtk_nfc_sys_ioctl_command(
    MTK_NFC_GPIO_E ePin,
    MTK_NFC_IOCTL_CMD_E eArg
)
{
   UINT32 result;
   result = ioctl(gInterfaceHandle, MTK_NFC_IOCTL_WRITE, ((ePin << 8) | (eArg)));
   if(result != MTK_NFC_SUCCESS)
   {
      NFCD("mtk_nfc_sys_ioctl_command : node,%d , ioctl fail: %d, %s\n", gInterfaceHandle, errno, strerror(errno));
   }
   else
   {
      NFCD("mtk_nfc_sys_ioctl_command : Success.");
   }
...
UINT16 mtk_nfc_sys_ioctl_set_param(MtkNfcDriver_ParamID_et ParamId,UINT16 SetValue)
{
    UINT16 ret = 0;
    UINT32 Arg = 0x80000000 | (((UINT32)ParamId & 0x7FFF) << 16) | SetValue;

    //NFCD("mtk_nfc_sys_ioctl_set_param Param %d ,SetValue %d  ret %d",(UINT32)ParamId,SetValue,ret);
    
    ret = ioctl(gInterfaceHandle, MTK_NFC_IOCTL_SET_GET, Arg);
...
UINT16 mtk_nfc_sys_ioctl_get_param(MtkNfcDriver_ParamID_et ParamId)
{
    UINT16 GetValue = 0;
    UINT32 Arg = (((UINT32)ParamId & 0x7FFF) << 16);

    //NFCD("mtk_nfc_sys_ioctl_get_param1 Param %d ,GetValue %d",(UINT32)ParamId,GetValue);
    
    GetValue = ioctl(gInterfaceHandle, MTK_NFC_IOCTL_SET_GET, Arg);

    //NFCD("mtk_nfc_sys_ioctl_get_param Param %d ,GetValue %d",(UINT32)ParamId,GetValue);
...
#define TX_PRINT_MAX    (256)
#define TX_DEBUG_BUF_LEN    (1024)  //2048

INT32 mtk_nfc_sys_i2c_write(UINT8* data, UINT16 len)
{
    INT32 result, i;
    CHAR pDeBugBuffer[TX_DEBUG_BUF_LEN];
    uint32_t DeBugL = 0;  

    NFCD("mtk_nfc_sys_i2c_write :len,0x%x\r\n", len);

    memset(pDeBugBuffer, 0x00, TX_DEBUG_BUF_LEN);    

    sprintf(pDeBugBuffer, "[TX](%d),", len);
    DeBugL = strlen(pDeBugBuffer);
...
VOID *mtk_nfc_sys_i2c_init(INT32 type)
{
   ...
   return (VOID *)(intptr_t)gInterfaceHandle;
}
...
//    mtk_nfc_sys_mutex_give(MTK_NFC_MUTEX_TIMER);
    ...
    {
        NFCD("[TIMER] Expired but Callback function is NULL.\n");
    }
}
#else
...
NFC_FILE mtk_nfc_sys_file_open(const CHAR *szFileName, UINT32 i4Mode)
{
    ...
    NFCD("mtk_nfc_sys_file_open, fopen start. file,%s, mode,%s. \r\n", szFileName, szMode);
    fp = fopen(szFileName, szMode);
    NFCD("mtk_nfc_sys_file_open, fopen stop. \r\n");

    if (fp != NULL)
    {
        return (NFC_FILE)(uintptr_t)fp;
    }
...
UINT32 mtk_nfc_sys_file_read (NFC_FILE hFile, void *DstBuf, UINT32 u4Length)
{
    if (hFile != 0)
    {
        return (UINT32)fread(DstBuf, 1, u4Length, (FILE *)(uintptr_t)hFile);
    }
...
{
    return fseek((FILE *)(uintptr_t)hFile, u4OffSet, u4Origin); 
}
...
{
    return (UINT32)ftell((FILE *)(uintptr_t)hFile);
}
...
INT32 mtk_nfc_sys_event_creates(void)
{
    INT32 result;
    ...
    result = mtk_nfc_sys_event_create(MTK_NFC_EVENT_2AOSPHAL);
    if (result != MTK_NFC_SUCCESS) {
        goto lbl_Abort1;
    }
...
lbl_Abort1:
    mtk_nfc_sys_event_delete(MTK_NFC_EVENT_2AOSPHAL);
lbl_Abort2:
    mtk_nfc_sys_event_delete(MTK_NFC_EVENT_2MAIN);
lbl_Abort3:
...
INT32 mtk_nfc_sys_event_deletes(void)
{
    INT32 result1,result2,result3;
    
    result1 = mtk_nfc_sys_event_delete(MTK_NFC_EVENT_2MAIN);

    result2 = mtk_nfc_sys_event_delete(MTK_NFC_EVENT_2SERV);

    result3 = mtk_nfc_sys_event_delete(MTK_NFC_EVENT_2AOSPHAL);

    if((result1 == MTK_NFC_SUCCESS)&&(result2 == MTK_NFC_SUCCESS)&&(result3 == MTK_NFC_SUCCESS))
    {
        return MTK_NFC_SUCCESS;
    }
